...
官方 Alpha.4 / rc.1 的读取接口支持 `ignorable`，但 `Session.append()` 不提供写入该信封标记的公开参数。模式状态保存在按 `Session` 对象索引的 WeakMap 中，不写自定义事件、不修改宿主事件目录。实际模型请求继续由 DSH 标准 `request/header` 记录完整系统提示。重启或重新加载插件后使用当前默认模式；活跃父会话的模式仍可继承。
