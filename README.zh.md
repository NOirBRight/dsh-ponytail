...
宿主 `@deepseek-ai/dsh-*` peer 与开发依赖使用无上界的下限范围 `>=0.1.7-alpha.2`。锁文件解析为已测试的 DSH `0.1.7-rc.1` 包。Cordis peer 与开发依赖范围为 `>=4.0.4 <5.0.0`；锁文件解析为 `4.0.4`。

`package.json#dsh.compatibility.dshReleases` 将 `0.1.7-alpha.2` 和 `0.1.7-rc.1` 标记为兼容版本。未知运行时仍会告警一次并尝试正常挂载；只有复现过的故障才会加入 blocklist。
...
只在 `~/.dsh-lab` / 3082 验收。不要修改生产 `~/.dsh` / 3080。

此 bundle 以官方 DSH `0.1.7-alpha.2` 和 `0.1.7-rc.1` 包的类型编译并通过了兼容验证。上游技能内容随 bundle 本地发布，运行时不访问网络。
...
`pnpm run check` 会运行单测、类型检查、Host/Web 构建、0.1.7-rc.1 Host/客户端加载器 smoke，以及 pack/install 检查。`scripts/sync-upstream.mjs` 从本地上游 checkout 更新 SKILL.md，不在运行时联网。
